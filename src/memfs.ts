...
import { dirNames, fileNames, fileContents } from './projectFiles';
...
export class MemFS implements FileSystemProvider, FileSearchProvider, TextSearchProvider, Disposable {
	static scheme = 'memfs';
    // TODO
	static project_name = 'solana-hello-world';
    static program_id = 'HELLOcdscdscd';
	static project_dir = '/home/wj/projects/solana-tx-debug/programs/' + MemFS.program_id + '/code';
    static start_file = 'src/lib.rs'
	...
	seed() {
        // Root
		this.createDirectory(Uri.parse('memfs:/'+MemFS.project_name+'/'));
        // All other dirs
        for (var dir_name of dirNames) {
		    this.createDirectory(Uri.parse('memfs:' + MemFS.project_name + '/' + dir_name));
        }
        // Files
        fileNames.forEach((file_name, i) => {
		    this.writeFile(Uri.parse('memfs:/' + MemFS.project_name + '/' + file_name), textEncoder.encode(fileContents[i]), { create: true, overwrite: true });
        });
	}
